...
export async function render(state, mod, dataView, windowSize, verticalZoomHeightProperty) { 
    ...
    function buildTemplates(sfdata, numberOfTracks) {
        depthCurveName = "DEPTH";
        depthUnit = "m";

        wellColumnName = "WELL";
        selectedWell = "a well" // getCurveData(0, wellColumnName, sfdata);
    ...
    function Initialize(div_id, templates, sfData) {
        ...
        $("#" + div_id).append(`
            ...
            <DIV id="accordionConf" style="clear: right;"></DIV>
            </DIV>
            `
    ...
    var numberOfTracks = await (await mod.property("trackVisibility")).value()
    numberOfTracks  = [...numberOfTracks].reduce((a,b)=>a+parseInt(b),0) - 0.5
    ...
    //R.  REPLACE tarck labels with doc props

    //R.1 Get document properties
    const labels = "GR,CAL,PHIN,PHID,RESD,ZONE,FACIES".split(",") //hard coded, sorry

   var props ="depth.blend.curve1,depth.blend.curve2,depth.blend.curve3,depth.blend.curve4,depth.blend.curve5,depth.blend.zone1,depth.blend.facies1".split(",");
//    var props ="depth.blend.curve1,,depth.blend.facies1".split(","); //test incomplete wellLogModTrackTitlesMapping doc prop format
    // var props = (await mod.document.property("wellLogModTrackTitlesMapping")).value().split(",") 

    //R.2 Map labels with doc props. If label not exists it will render as ⚠ 
    Promise.all(props.map(async (p) => {
        ...
        var x = [...document.querySelectorAll(".trackHeaderDivContent .header>text")]
        x.forEach((x,i)=>{
//              x.innerHTML = x.innerHTML.replace(labels[i],v[i]==undefined?"⚠":v[i])
        });
    })


    //H. Hide tracks based on doc prop. The doc prop should be a list of 1 or 0 to indicate which tarck are visible. i.e. 11101011  4th and 6th are hidden

    //H.1 Get trackVisibility mod property
    // let visibleTrackSettings = "1,1,0,0,0,1".split(",").map(x=>{return x=="1"})
    let visibleTrackSettings = [...(await (mod.property("trackVisibility"))).value()]
    console.log(visibleTrackSettings,(await (mod.property("trackVisibility"))).value())
    ///visibleTrackSettings = visibleTrackSettings.value().split(",").map(x=>{return x=="1"})

    //H.2 Hide tracks
    document.querySelectorAll("[id^='mod-containerTrackHolder']").forEach((t,i)=>{ 
        if(!visibleTrackSettings[i]) t.style.display='none'
    })

    //H.3 Fit visible tracsk to width
    //TBD

    //instead of hiding tracks, how about deleting tracks when clicking accordion settings button 
    ...
    mod.property("trackDocPropMappings").then(q=>{
        document.getElementById("trackMmapping").value = q.value()
    });
    ...
    document.getElementById("setSettings")?.addEventListener("click",()=>{ 
        ...
        mod.property("trackDocPropMappings").then(p=>{p.set(document.getElementById("trackMmapping").value)})
...
function insertDropdown(divContent, i, k, templates, name, sfData, mod) {
    var blackImgSrc =
    ...
    $("#dropdown" + name + "_" + i + "_" + k).ddslick({
        ...
        onSelected: function (data) {
            ...
            PropertyOnChange("mod-container", i, k, templates, sfData, selData, name, mod);
        }
    });
}

function insertTextInput(divContent, i, k, templates, propName, sfData, mod) {
    var selectedData = "";
    ...
    var textInput = divContent
        ...
        .on("input", function (d) {
            InputOnChange(divContent, i, k, templates, sfData, this.value, propName, mod);
        });
}

function InputOnChange(div_id, i, k, templates, sfData, selectedData, propName, mod) {
    if (templates[i] && initialized) {
        ...
        let result_1 = multipleLogPlot("mod-container", templates, sfData, mod);
    }
}

function PropertyOnChange(div_id, i, k, templates, sfData, selectedData, propName, mod) {
    if (templates[i] && initialized) {
        var template = templates[i];
        let template_components = template[0]["components"];
        let templateCurves = template_components[0]["curves"];
        ...
        multipleLogPlot("mod-container", templates, sfData, mod);
    }
}

function accordionTemplate(templates, i, sfData, mod) {
    let template = templates[i];
    let template_components = template[0]["components"];
    let templateCurves = template_components[0]["curves"][0];
    ...
    if (templateCurves["dataType"] == "curve") {
        ...
        var tab = content.append("div").attr("id", "tabs_" + i);

        var ul = tab.append("ul");
        ...
        for (let k = 0; k < curveNames.length; k++) {
            if (curveNames[k]) {
                var tab = tab.append("div").attr("id", "tab_" + i + "_" + k);
                ...
                insertDropdown(divItem, i, k, templates, "Thickness", sfData, mod);
                ...
                insertDropdown(divItem, i, k, templates, "LineStyle", sfData, mod);
                ...
                insertDropdown(divItem, i, k, templates, "LineColor", sfData, mod);
                ...
                insertDropdown(divItem, i, k, templates, "AreaFill", sfData, mod);
                ...
                divItem.text("Color:").append("br");
                insertDropdown(divItem, i, k, templates, "AreaColor", sfData, mod);
                ...
                insertDropdown(divItem, i, k, templates, "ScaleType", sfData, mod);
                ...
                insertTextInput(divItem, i, k, templates, "ScaleMin", sfData , mod);
                ...
                insertTextInput(divItem, i, k, templates, "ScaleMax", sfData, mod);
                ...
                insertTextInput(divItem, i, k, templates, "CutoffShaleSilt", sfData, mod);
                ...
                insertTextInput(divItem, i, k, templates, "CutoffSiltSand", sfData, mod);
        ...
        if (i==3){
            ...
            //get from mod properties
            // var mappingValues = "depth.blend.curve1,depth.blend.curve2,depth.blend.curve3,depth.blend.curve4,depth.blend.curve5,depth.blend.zone1,depth.blend.facies1"
            // var visibilityValues = "1,1,1,0,0,1"


            //add last accordion pane
            d3.select("#accordionConf")
            .append("h3")
            .text("Options")

            // //add mappig & visibility tab
            var optionContents = d3.select("#accordionConf").append("div")
            
            
            // add mapping and visibility settings
            optionContents.append("div").attr("class","controlGroup")
            .append("p")
            fieldset = optionContents.append("fieldset");
            fieldset.append("legend").text("Mapping & Visibility")
            ...
            divItem
            .append("div").text("Document Property Mappings ")
            var input1 = divItem
            .append("input")
            .attr("type", "text")
            .attr("id", "mappings")
            .style("width", "250px")
            .attr("title","csv document property holding values for track titles")

            divItem.append("br")
            divItem.append("br")

            divItem
            .append("div").text("Track visibility settings")
            var input2 = divItem
            .append("input")
            .attr("type", "text")
            .attr("id", "visibility")
            .style("width", "250px")
            .attr("title","csv values 1|0 to indicate which tracks are visible")

            
            //populate inputs
            mod.property("trackVisibility").then(p=>{
                document.getElementById("visibility").value = p.value()
            });
            mod.property("trackDocPropMappings").then(p=>{
                document.getElementById("mappings").value = p.value()
            });
            
            optionContents.append("center").append("button").text("Set").attr("style","align:center").on("click",()=>{ 
                mod.property("trackVisibility").then(p=>{p.set(document.getElementById("visibility").value)})
                mod.property("trackDocPropMappings").then(p=>{p.set(document.getElementById("mappings").value)})
            })
...
//delete accordingly
var toDelete = [...await (await mod.property("trackVisibility")).value()]
console.log(toDelete)
templates = templates.filter((x,i) => parseInt(toDelete[i]))


//    templates = templates.slice(2,5) 
    ...
    for (let i = 0; i < templates.length; i++) {
        ...
        if (updateAccordionTools) {
            for (let i = 0; i < templates.length; i++) {
                if (templates[i]) {
                    accordionTemplate(templates, i, sfData, mod);
                }
            }
...
/*    var editor = ace.edit("trackMmapping",{
        mode: 'ace/mode/json',
        selectionStyle: 'text',
